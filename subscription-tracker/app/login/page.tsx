  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        setError("Firebase niet geconfigureerd. Zet NEXT_PUBLIC_FIREBASE_* in .env.local");
      }
    } catch (err) {
      ...
      <div className="w-full max-w-sm rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div className="mt-6">
          {hasFirebase ? (
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:opacity-50"
            >
              {loading ? (
                "Bezig..."
              ) : (
        ...
        <p className="mt-6 text-center">
          <Link href="/" className="text-sm text-slate-600 hover:text-slate-900">
            ← Terug naar dashboard (zonder login)
          </Link>
        </p>
      </div>
    </div>
